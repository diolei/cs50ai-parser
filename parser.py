...
PP -> P N | P Det N | P Det Adj N
RC -> R V NP | R V Det N | R V Det Adj N
VP -> V | Adv VP | V Adv | VP NP | V NP Adv
AP -> Adj | Det Adj | Det Adj N | Adj N | AP Adj
"""
...
def preprocess(sentence):
    ...
    for token in tokens:
        if any(char.isalpha() for char in token):
            valid_words.append(token.lower())
    return valid_words
...
def np_chunk(tree):
    ...
    for subtree in parent.subtrees():
        # If subtree is labelled as a N the parent must be a NP
        if subtree.label() == "N":
